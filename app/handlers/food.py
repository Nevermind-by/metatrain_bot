...
async def saved_recipe(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not value.isdigit():
        await callback.answer("Некорректное блюдо", show_alert=True); return
    user_id = await _user_id(callback.from_user.id)
    recipe = await recipe_repository.get(user_id, int(value)) if user_id is not None else None
    if recipe is None:
        await callback.answer("Блюдо не найдено", show_alert=True); return
...
async def food_grams(message: Message, state: FSMContext) -> None:
    ...
    if all(k in data for k in ("calories", "protein", "fat", "carbohydrates")):
        ...
        return
    await state.update_data(grams=grams); await state.set_state(FoodStates.calories)
...
async def recipe_servings(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    try: servings = float((message.text or "").replace(",", "."))
    except ValueError: await message.answer("Введи число, например: 1.5", reply_markup=navigation_keyboard()); return
    if servings <= 0: await message.answer("Количество порций должно быть больше нуля.", reply_markup=navigation_keyboard()); return
    user_id = await _user_id(message.from_user.id) if message.from_user else None
    if user_id is None: await state.clear(); await message.answer("Пользователь не найден. Используй /start.", reply_markup=navigation_keyboard()); return
...
async def _numeric(message: Message, state: FSMContext, next_state: object, key: str, prompt: str) -> None:
    ...
    except ValueError: await message.answer("Введи число, например: 12.5", reply_markup=navigation_keyboard()); return
    if value < 0: await message.answer("Значение не может быть отрицательным.", reply_markup=navigation_keyboard()); return
    await state.update_data(**{key: value}); await state.set_state(next_state); await message.answer(prompt, reply_markup=navigation_keyboard())
...
async def food_carbohydrates(message: Message, state: FSMContext) -> None:
    try: value = float((message.text or "").replace(",", "."))
    except ValueError: await message.answer("Введи число, например: 20", reply_markup=navigation_keyboard()); return
    if value < 0: await message.answer("Значение не может быть отрицательным.", reply_markup=navigation_keyboard()); return
    data = await state.get_data(); await state.update_data(carbohydrates=value); await state.set_state(FoodStates.grams)
...
async def today_handler(message: Message) -> None:
    ...
    if not entries: await message.answer("Сегодня пока ничего не записано.", reply_markup=navigation_keyboard()); return
    lines = ["📅 <b>Сегодня</b>", ""]
    for entry in entries: lines.append(f"#{entry.id} {MEALS[entry.meal]}: {entry.product_name} — {entry.quantity:g} {entry.unit} ({entry.calories:g} ккал)")
    lines += ["", f"🔥 {totals['calories']:g} ккал", f"🥩 Б {totals['protein']:g} г", f"🥑 Ж {totals['fat']:g} г", f"🍚 У {totals['carbohydrates']:g} г"]
    if profile: lines += ["", f"🎯 Цель: {profile.calories} ккал", f"Осталось: {max(0, profile.calories - totals['calories']):g} ккал"]
...
async def delete_food(message: Message) -> None:
    ...
    if len(parts) != 2 or not parts[1].isdigit(): await message.answer("Используй: /delete_food <id>", reply_markup=navigation_keyboard()); return
