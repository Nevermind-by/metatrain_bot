...
async def workout_name(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(f"🏋️ {workout.name}\n\nПервое упражнение?\nНапример: Жим лёжа\n\nКогда закончишь упражнение — введи следующее название.\n/cancel — отменить")
...
async def workout_reps(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(reps=reps); await state.set_state(WorkoutStates.rpe); await message.answer("RPE от 1 до 10? Можно пропустить: 0")
...
async def workout_rpe(message: Message, state: FSMContext) -> None:
    ...
    if rpe != 0 and not 1 <= rpe <= 10: await message.answer("RPE должен быть от 1 до 10, либо 0 чтобы пропустить."); return
    ...
    await message.answer(f"Подход {next_set}: вес в кг?\n\nИли введи /next, чтобы перейти к следующему упражнению.")
...
@router.message(WorkoutStates.exercise, Command("finish"))
async def finish_workout(message: Message, state: FSMContext) -> None:
    data = await state.get_data(); workouts = await workout_service.recent(int(data.get("user_id", 0))) if data.get("user_id") else []
