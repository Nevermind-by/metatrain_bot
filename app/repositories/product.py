...
class ProductRepository:
    async def create(self, product: Product) -> Product:
        async with await get_connection() as connection:
            cursor = await connection.execute(
                ...
                (user_id, name, calories, protein, fat, carbohydrates)
                VALUES (?, ?, ?, ?, ?, ?)""",
                (product.user_id, product.name, product.calories, product.protein,
                 product.fat, product.carbohydrates),
            )
    ...
    async def list_recent(self, user_id: int, limit: int = 10) -> list[Product]:
        async with await get_connection() as connection:
            cursor = await connection.execute(
                "SELECT * FROM products WHERE user_id = ? ORDER BY id DESC LIMIT ?",
                (user_id, limit),
            )
            rows = await cursor.fetchall()
        return [self._to_model(row) for row in rows]
