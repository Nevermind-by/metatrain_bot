...
async def init_database() -> None:
    async with await get_connection() as connection:
        ...
        await connection.execute("""CREATE TABLE IF NOT EXISTS user_profiles (
            user_id INTEGER PRIMARY KEY, gender TEXT NOT NULL, age INTEGER NOT NULL, height_cm REAL NOT NULL,
            weight_kg REAL NOT NULL, activity_level TEXT NOT NULL DEFAULT 'sedentary', goal TEXT NOT NULL,
            calories INTEGER NOT NULL, protein INTEGER NOT NULL, fat INTEGER NOT NULL, carbohydrates INTEGER NOT NULL,
            created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)""")
        columns = await connection.execute_fetchall("PRAGMA table_info(user_profiles)")
        if "activity_level" not in {row[1] for row in columns}:
            await connection.execute("ALTER TABLE user_profiles ADD COLUMN activity_level TEXT NOT NULL DEFAULT 'sedentary'")
        await connection.execute("""CREATE TABLE IF NOT EXISTS products (
            ...
            fat REAL NOT NULL CHECK (fat >= 0), carbohydrates REAL NOT NULL CHECK (carbohydrates >= 0),
        ...
        await connection.execute("""CREATE TABLE IF NOT EXISTS recipes (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, name TEXT NOT NULL,
            servings INTEGER NOT NULL CHECK (servings > 0), calories REAL NOT NULL CHECK (calories >= 0),
            protein REAL NOT NULL CHECK (protein >= 0), fat REAL NOT NULL CHECK (fat >= 0),
            carbohydrates REAL NOT NULL CHECK (carbohydrates >= 0), UNIQUE(user_id, name),
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)""")
        await connection.execute("""CREATE TABLE IF NOT EXISTS recipe_ingredients (
            id INTEGER PRIMARY KEY AUTOINCREMENT, recipe_id INTEGER NOT NULL, product_id INTEGER NOT NULL,
            grams REAL NOT NULL CHECK (grams > 0), UNIQUE(recipe_id, product_id),
        ...
        await connection.execute("""CREATE TABLE IF NOT EXISTS weight_entries (
            ...
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)""")
        await connection.execute("""CREATE TABLE IF NOT EXISTS workout_entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, name TEXT NOT NULL,
            duration_minutes INTEGER, calories_burned INTEGER, notes TEXT, performed_at TEXT NOT NULL,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)""")
